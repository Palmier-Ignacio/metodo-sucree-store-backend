...
export async function sendContactMessage(req, res, next) {
  try {
    ...
    await transporter.sendMail({
      from: `Formulario web <${env.gmailUser}>`,
      to: env.contactReceiverEmail,
      replyTo: `${fullName} <${email}>`,
      subject: `Nuevo contacto web - ${safeSubject}`,
      html: `
      ...
      <div style="background:#ffffff;border-radius:24px;overflow:hidden;border:1px solid rgba(198,167,105,.35);box-shadow:0 12px 36px rgba(74,46,43,.10);">
        ...
        <div style="padding:30px;">
          ...
          <div style="margin-top:28px;text-align:center;">
            <a href="mailto:${safeEmail}" style="display:inline-block;padding:14px 24px;border-radius:999px;background:#D8A7B1;color:#4A2E2B;text-decoration:none;font-weight:bold;">
